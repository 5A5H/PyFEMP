...
def Elmt_KS(XI,UI,Hn,Ht,Mat,dt):
    ...
    T1ddt_n = Hn[10];
    T2ddt_n = Hn[11];
    if (verbose): print('U_n :',[U1_n,U2_n])
    if (verbose): print('V_n :',[V1_n,V2_n])
    if (verbose): print('A_n :',[A1_n,A2_n])
    # material parameter
    Emod  = Mat[0];
    Area  = Mat[1];
    mue   = Mat[2];
    ...
    if (verbose): print('U :',[U1,U2])
    if (verbose): print('V :',[V1,V2])
    if (verbose): print('A :',[A1,A2])
    ...
    N = 1.0/2.0 * array([1.0, 1.0]);
    B = 1.0/2.0 * array([-1.0, 1.0]) * dxidx;
    # Element quantities
    A      = N[0] * A1 + N[1] * A2;
    Eps    = B[0] * U1 + B[1] * U2;
    ...
    # Element matrix
    k_e[0][0] += L * Area * Emod*B[0]*B[0] + L * Area * mue*(gamma/(beta*dt))*B[0]*B[0] + L * Area * rho*(1.0/(beta*dt**2))*N[0]*N[0];
    k_e[0][2] += L * Area * Emod*B[1]*B[0] + L * Area * mue*(gamma/(beta*dt))*B[1]*B[0] + L * Area * rho*(1.0/(beta*dt**2))*N[1]*N[0];
    k_e[2][0] += L * Area * Emod*B[0]*B[1] + L * Area * mue*(gamma/(beta*dt))*B[0]*B[1] + L * Area * rho*(1.0/(beta*dt**2))*N[0]*N[1];
    k_e[2][2] += L * Area * Emod*B[1]*B[1] + L * Area * mue*(gamma/(beta*dt))*B[1]*B[1] + L * Area * rho*(1.0/(beta*dt**2))*N[1]*N[1];
    # BaEn static term 
    ...
    #if (verbose): print('fact : ',(1.0/(beta*dt**2)))
    #if (verbose): print('L : ',L*Area)
    #if (verbose): print('fact : ',N)
    if (verbose): print('r_e : ',r_e)
    if (verbose): print('k_e : ')
    if (verbose): print(k_e)
    return r_e, k_e
...
def Elmt_Init():
    ...
    return NoElementNodes, ElementDofNames, NoElementHistory, ElementMaterialNames
...
#test the element
#r,k = Elmt_KS([0,10],[0,0,0,0],zeros(12),zeros(12),[21e3,1,100,1,1,0.001],0.1);
#print(k);
