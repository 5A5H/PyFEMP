...
class FEM1D:
  ...
  def __init__(self,Element):
    ...
    self.ElementMaterialNames = Element.Elmt_Init()
  ...
  def Add_Mesh(self, DomainLength, NumberOfElements):
    ...
    self.XI = np.linspace(0.0, self.GlobalDomainLength, num=self.NoElements + 1)
  ...
  def Add_Material(self, MaterialList, Option = None):
      ...
      if len(MaterialList) != self.NoElementMaterial:
        print('Error: Number of material parameter does not fit element requirements!')
  ...
  def Add_EBC(self,NodeSelector,DofSelector,Value):
    ...
    if AffectedDof >= self.NoNodeDofs:
      print("Error: Nodal degrees of freedom do not exceed %i"%self.NoNodeDofs)
      ...
      for i in range(len(Input)):
        if isinstance(Input[i], int):
          if Input[i] <= self.NoElements:
            ...
            print('Error: %i is not a valid node number!'%Input[i])
            return
        else:
          print('Error: ',Input[i]," is not a valid node number! Integer required.")
  ...
  def NextStep(self, time=1, lambda_load=1):
    ...
    if self.dt <1e-16:
      print('Error: Time given in NextStep is smaller than internal time: %f5.2'%time)
      return
    self.time  = time
    self.step  += 1
    self.h_n   = self.h_t
  ...
  def CallElement(self,i):
    if i > self.NoElements:
      print('Error: Input exceeds number of elements. max is : %i8'%self.NoElements)
    ...
    Elmt_UI  = self.DI[elmt_dof_start:elmt_dof_start+self.NoElementNodes*self.NoNodeDofs];
    Elmt_Hn  = self.h_n[elmt_hist_start:elmt_hist_start+self.NoElementHistory];
    ...
    self.h_t[elmt_hist_start:elmt_hist_start+self.NoElementHistory] = Elmt_Ht;
    if (self.verbose):
  ...
  def Assemble(self):
    ...
    h_t = np.zeros(self.NoElements * self.NoElementHistory);
  ...
  def Mod_Material(self, MaterialList, Element_I):
    ...
    if Element_I > len(self.ElementMaterial) -1:
      print('Error: Material of Element %i is not defined yet.'%i)
      return 
